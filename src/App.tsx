...
import { Activity, ArrowUpRight, BadgeCheck, ExternalLink, LockKeyhole, RadioTower, Wallet } from "lucide-react";
...
import { allocatorAddress, getReceiptExplorerUrl, MEZO_NETWORK } from "./lib/mezo";
...
function App() {
  ...
  const [receipt, setReceipt] = useState<VoteReceipt>(() => buildFixtureReceipt(initialGauges, initialPosition));
  const impact = useMemo(() => calculateImpact(gauges, position), [gauges, position]);
  const explorerUrl = getReceiptExplorerUrl(receipt.txHash);

  const mode = allocatorAddress ? "Allocator address configured" : "Fixture demo mode";
  ...
  return (
    <main className="min-h-screen bg-graphite text-ivory">
        ...
        <div className="relative z-10 mx-auto flex min-h-[calc(100vh-32px)] max-w-7xl flex-col rounded-lg border border-white/10 bg-black/45 shadow-glass backdrop-blur">
          <nav className="flex items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-white">Mezo Allocator</p>
                <p className="text-xs text-white/45">Gauge simulator and receipt workbench</p>
              </div>
            </div>
            <div className="hidden items-center gap-2 md:flex">
              <StatusPill icon={<BadgeCheck size={14} />} label={mode} />
              <StatusPill icon={<Activity size={14} />} label={`Chain ${MEZO_NETWORK.id ?? 31611}`} />
            </div>
          </nav>
          ...
          <div className="grid flex-1 gap-8 px-4 pb-6 sm:px-6 lg:grid-cols-[0.9fr_1.35fr] lg:px-8 lg:pb-8">
            ...
            >
              <motion.p variants={fadeUp} className="mb-5 text-xs font-semibold text-btc">
                MEZO Utilization track
              </motion.p>
              <motion.h1 variants={fadeUp} className="max-w-2xl text-[42px] font-semibold leading-[0.98] text-white sm:text-[58px] lg:text-[72px]">
                MEZO becomes the dial that steers{" "}
                <span className="font-serif italic font-normal text-musd">Bitcoin-backed</span> capital.
              </motion.h1>
              <motion.p variants={fadeUp} className="mt-5 max-w-xl text-base leading-7 text-white/62 sm:text-lg">
                Model veBTC base power, MEZO boost, and gauge votes. Then produce a visible `AllocationUpdated`
                receipt instead of a dashboard-only promise.
              </motion.p>
              <motion.div variants={fadeUp} className="mt-7 flex flex-wrap gap-3">
                <button onClick={onSimulate} className="inline-flex items-center gap-2 rounded-full bg-ivory px-5 py-3 text-sm font-semibold text-black transition hover:bg-white">
                  Simulate Allocation <ArrowUpRight size={16} />
                </button>
                <a
                  href={MEZO_NETWORK.faucetUrl}
                  className="inline-flex items-center gap-2 rounded-full liquid-glass px-5 py-3 text-sm font-semibold text-white"
                >
                  Testnet faucet <ExternalLink size={15} />
                </a>
              </motion.div>

              <motion.div variants={fadeUp} className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
                <Metric icon={<LockKeyhole size={16} />} label="veBTC base" value={impact.baseVotingPower.toFixed(3)} />
                <Metric icon={<Wallet size={16} />} label="MEZO boost" value={`${impact.boostMultiplier.toFixed(2)}x`} />
                <Metric icon={<Activity size={16} />} label="Power" value={impact.effectiveVotingPower.toFixed(3)} />
              </motion.div>
            </motion.div>

            <div className="flex flex-col justify-end gap-4">
              <GaugeConstellation gauges={gauges} leadingGaugeId={impact.leadingGaugeId} />
              <AllocatorWorkbench
                gauges={gauges}
                position={position}
                impact={impact}
                hasAllocator={Boolean(allocatorAddress)}
      ...
      <section className="mx-auto grid max-w-7xl items-start gap-4 px-4 pb-16 sm:px-6 lg:grid-cols-[1fr_0.8fr] lg:px-8">
        ...
        <ReceiptPanel receipt={receipt} explorerUrl={explorerUrl} allocatorAddress={allocatorAddress} />
      </section>
    </main>
  );
}
